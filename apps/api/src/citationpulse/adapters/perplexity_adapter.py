"""Perplexity engine adapter.

Routed through OpenRouter (`perplexity/sonar` by default). Sonar already does
native web search, so we don't add the `:online` suffix; OpenRouter forwards
Perplexity's `citations` array which the LLMRouter normalizes for us.
"""

from __future__ import annotations

import json
import time
from typing import Any

from citationpulse.adapters.base import BaseEngineAdapter, EngineResponse, RawCitation
from citationpulse.core.config import get_settings
from citationpulse.models.domain import EngineType
from citationpulse.services.llm_router import LLMConfigError, get_router
from citationpulse.storage.r2 import upload_raw_payload


class PerplexityAdapter(BaseEngineAdapter):
    def __init__(self) -> None:
        super().__init__(EngineType.PERPLEXITY)

    async def run(self, prompt: str, locale: str, run_ctx: dict[str, Any]) -> EngineResponse:
        settings = get_settings()
        t0 = time.perf_counter()
        if not settings.openrouter_api_key:
            return EngineResponse("", [], "", int((time.perf_counter() - t0) * 1000), None)

        model = settings.perplexity_model or "perplexity/sonar"
        try:
            resp = await get_router().chat_completion(
                model=model,
                messages=[{"role": "user", "content": f"[{locale}] {prompt}"}],
            )
        except LLMConfigError:
            return EngineResponse("", [], "", int((time.perf_counter() - t0) * 1000), None)

        key = f"raw/{run_ctx.get('run_id','unknown')}/openrouter_perplexity.json"
        upload_raw_payload(key, json.dumps(resp.raw, default=str).encode(), "application/json")
        cites = [
            RawCitation(url=c.url, snippet=c.snippet, position=c.position)
            for c in resp.citations
        ]
        return EngineResponse(
            answer_text=resp.text,
            citations=cites,
            raw_payload_ref=key,
            latency_ms=resp.latency_ms,
            cost_usd=resp.cost_usd,
        )
